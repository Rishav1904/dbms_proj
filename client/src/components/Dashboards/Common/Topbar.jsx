...
function Topbar({ name, notifications: initialNotifications }) {
  ...
  return (
    <div className="py-5 px-5 flex items-center justify-end text-white w-full bg-primary shadow-lg fixed top-0 right-0 z-50 md:w-[calc(100%-256px)] md:ml-[256px]">
      <div className="flex gap-3">
        <Link to="settings">
          <svg
            ...
            strokeWidth={1.5}
            stroke="currentColor"
            className="w-6 h-6 hover:text-accent transition-colors duration-300"
          >
            <path
              strokeLinecap="round"
            ...
            <ul className="[&>*:nth-child(1)]:border-t-0">
              {notifications.map((noti) => (
                ...
                >
                  ...
                  <span 
                    className="group/edit relative flex cursor-pointer"
                    onClick={async () => {
                      try {
                        const response = await fetch("http://localhost:3000/api/request/mark-read", {
                          method: "POST",
                          headers: {
                            "Content-Type": "application/json",
                          },
                          body: JSON.stringify({ usn_no: noti })
                        });
                        const data = await response.json();
                        if (data.success) {
                          // Remove the notification from the list
                          setNotifications(prevNotifications => prevNotifications.filter(n => n !== noti));
                        }
                      } catch (error) {
